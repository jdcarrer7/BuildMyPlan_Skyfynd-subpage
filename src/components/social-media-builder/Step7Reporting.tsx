...
import { reportingOptions } from '@/data/socialMediaBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step7Reporting() {
  const { reporting, setReporting } = useSocialMediaBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">How do you want to track performance?</h2>
        <p className="text-[var(--text-secondary)]">
          Select your preferred reporting frequency and analytics access.
        </p>
      </div>
