...
import { strategyOptions } from '@/data/socialMediaBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step6Strategy() {
  const { strategy, setStrategy } = useSocialMediaBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  // Format price display
  const formatPrice = (option: typeof strategyOptions[0]) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What level of strategic support do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Choose how much strategic planning and guidance you want for your social media.
        </p>
      </div>
