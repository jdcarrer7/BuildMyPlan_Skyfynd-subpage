...
import { useUnifiedQuoteStore, serviceMetadata, WebsiteConfig, AppConfig, AnimationConfig, ImageConfig, SoundConfig, PaidMediaConfig, EmailMarketingConfig, VisualIdentityConfig, AIReceptionistConfig } from '@/hooks/useUnifiedQuoteStore';
import {
...
type ExpandedCard = 'social-media' | 'website' | 'app' | 'animation' | 'image' | 'sound' | 'paid-media' | 'email-marketing' | 'visual-identity' | 'ai-receptionist' | null;
...
export default function Step10Summary({ showQuoteForm = false, onCloseQuoteForm }: Step10SummaryProps) {
  const store = useSocialMediaBuilderStore();
  const unifiedStore = useUnifiedQuoteStore();
  const router = useRouter();

  const [expandedCard, setExpandedCard] = useState<ExpandedCard>('social-media');
  const [showAddServices, setShowAddServices] = useState(false);

  // Save to unified quote on mount
  useEffect(() => {
    // Only save if managementGoal is set (meaning we have a valid configuration)
    if (store.managementGoal) {
      store.saveToUnifiedQuote();
    }
  }, [store.managementGoal, store.saveToUnifiedQuote]);

  // Get labels
  const goalLabel = managementGoalOptions.find((g) => g.id === store.managementGoal)?.label;
  const platformLabel = platformOptions.find((p) => p.id === store.platforms)?.label;
  const frequencyLabel = frequencyOptions.find((f) => f.id === store.frequency)?.label;
  const contentLabel = contentOptions.find((c) => c.id === store.content)?.label;
  const engagementLabel = engagementOptions.find((e) => e.id === store.engagement)?.label;
  const strategyLabel = strategyOptions.find((s) => s.id === store.strategy)?.label;
  const reportingLabel = reportingOptions.find((r) => r.id === store.reporting)?.label;
  const storiesLabel = storiesOptions.find((s) => s.id === store.stories)?.label;
  const reelsLabel = reelsOptions.find((r) => r.id === store.reels)?.label;
  const durationLabel = durationOptions.find((d) => d.id === store.duration)?.label;

  // Get available and configured services
  const availableServices = unifiedStore.getAvailableServices().filter((s) => s !== 'social-media');
  const configuredServices = unifiedStore.getAllConfiguredServices();
  const otherServices = configuredServices.filter((s) => s.type !== 'social-media');
  ...
  const handleClear = () => {
    ...
    const remainingServices = configuredServices.filter(s => s.type !== 'social-media');

    // Clear from unified quote store first, then reset builder store
    unifiedStore.clearServiceConfig('social-media');
    store.resetBuilder();
  ...
  return (
    <div className="space-y-6">
      {/* Main Social Media Summary Card */}
      <div className="card p-6">
        ...
        <div className={`grid transition-all duration-300 ease-in-out ${expandedCard === 'social-media' ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
          <div className="overflow-hidden">
            <div className="space-y-4 mt-6">
                <SummarySection title="Management Goal" icon={<Target className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={1}>
                  <p className="text-[var(--text-secondary)]">{goalLabel}</p>
                </SummarySection>

                <SummarySection title="Platforms" icon={<Globe className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={2}>
                  <div className="flex justify-between">
                    <span className="text-[var(--text-secondary)]">{platformLabel}</span>
                    {store.platformsPrice !== null ? (
                      <span className="text-[var(--accent-pink)]">${store.platformsPrice}/mo</span>
                    ) : (
                ...
                <SummarySection title="Posting Frequency" icon={<FileText className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={3}>
                  <div className="flex justify-between">
                    <span className="text-[var(--text-secondary)]">{frequencyLabel}</span>
                    {store.frequencyPrice !== null ? (
                      <span className="text-[var(--accent-pink)]">${store.frequencyPrice}/mo</span>
                    ) : (
                      <span className="text-[var(--accent-orange)]">Custom Quote</span>
                    )}
                  </div>
                ...
                <SummarySection title="Content Creation" icon={<Lightbulb className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={4}>
                  <div className="flex justify-between">
                    <span className="text-[var(--text-secondary)]">{contentLabel}</span>
                    {store.contentPrice > 0 ? (
                      <span className="text-[var(--accent-pink)]">${store.contentPrice}/mo</span>
                    ) : (
                ...
                <SummarySection title="Engagement" icon={<MessageCircle className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={5}>
                  <div className="flex justify-between">
                    <span className="text-[var(--text-secondary)]">{engagementLabel}</span>
                    {store.engagementPrice !== null ? (
                      store.engagementPrice > 0 ? (
                        <span className="text-[var(--accent-pink)]">${store.engagementPrice}/mo</span>
                      ) : (
                        <span className="text-[var(--text-muted)]">$0</span>
                      )
                    ) : (
                ...
                <SummarySection title="Strategy" icon={<Users className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={6}>
                  <div className="flex justify-between">
                    <span className="text-[var(--text-secondary)]">{strategyLabel}</span>
                    {store.strategyPrice > 0 ? (
                      <span className="text-[var(--accent-pink)]">${store.strategyPrice}/mo</span>
                    ) : (
                      <span className="text-[var(--text-muted)]">$0</span>
                    )}
                  </div>
                ...
                <SummarySection title="Reporting" icon={<BarChart3 className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={7}>
                  <div className="flex justify-between">
                ...
                <SummarySection title="Add-ons" icon={<Zap className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={8}>
                  {store.stories === 'none' && store.reels === 'none' && store.selectedAdditionalAddOns.length === 0 ? (
                    <p className="text-[var(--text-muted)] text-sm">No add-ons selected</p>
                  ) : (
                    <div className="space-y-2">
                      {store.stories !== 'none' && (
                        <div className="flex justify-between text-sm">
                          <span className="text-[var(--text-secondary)]">{storiesLabel}</span>
                          <span className="text-[var(--accent-pink)]">${store.storiesPrice}/mo</span>
                        </div>
                      )}
                      {store.reels !== 'none' && (
                        <div className="flex justify-between text-sm">
                          <span className="text-[var(--text-secondary)]">{reelsLabel}</span>
                          {store.reelsPrice !== null ? (
                            <span className="text-[var(--accent-pink)]">${store.reelsPrice}/mo</span>
                          ) : (
                            <span className="text-[var(--accent-orange)]">Custom Quote</span>
                          )}
                        </div>
                      )}
                      {store.selectedAdditionalAddOns.map((addOn) => {
                        const addOnData = additionalServicesOptions.find((a) => a.id === addOn.id);
                        return (
                          <div key={addOn.id} className="flex justify-between text-sm">
                            <span className="text-[var(--text-secondary)]">{addOnData?.label}</span>
                            <span className={addOn.oneTime ? 'text-white' : 'text-[var(--accent-pink)]'}>
                              ${addOn.price}{addOn.oneTime ? '' : '/mo'}
                            </span>
                          </div>
                        );
                      })}
                ...
                <SummarySection title="Duration" icon={<Calendar className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={9}>
                  <div className="flex justify-between">
      ...
      {otherServices.filter((s) => s.type === 'email-marketing').map((service) => {
        ...
        const durationLabelEM = emailDurationOptions.find((d) => d.id === config.duration)?.label;
              ...
              <div className="overflow-hidden">
                <div className="space-y-4 mt-6">
                  ...
                  <div className="p-4 bg-[var(--bg-secondary)] rounded-lg">
                    ...
                    <div className="flex justify-between">
                      <span className="text-[var(--text-secondary)]">{durationLabelEM}</span>
                      <span className="text-[var(--text-muted)]">{config.durationMonths} months</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="overflow-hidden">
                <div className="space-y-4 mt-6">
                  ...
                  <div className="p-4 bg-[var(--bg-secondary)] rounded-lg">
                    ...
                    <div className="p-4 bg-[var(--bg-secondary)] rounded-lg">
                      <div className="flex items-center gap-2 mb-3">
                        <Lightbulb className="w-5 h-5 text-[var(--accent-purple)]" />
