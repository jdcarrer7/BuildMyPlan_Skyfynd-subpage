...
import { platformOptions } from '@/data/socialMediaBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step2Platforms() {
  const { platforms, setPlatforms } = useSocialMediaBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">Which social platforms do you need managed?</h2>
        <p className="text-[var(--text-secondary)]">
          Select the number of platforms you want us to manage for your business.
        </p>
      </div>
            ...
            <div key={option.id} className="relative">
              ...
              >
                <div className="flex flex-col gap-2">
                  ...
                  <div className="mt-1">
                    {option.customQuote ? (
                      <span className="text-[var(--accent-orange)] text-sm font-medium">Custom Quote</span>
                    ) : option.startsAt ? (
                      <span className="text-[var(--accent-blue)] text-sm font-medium">
                        Starts at ${option.price}/mo
                      </span>
                    ) : (
                      <span className="text-[var(--accent-blue)] text-sm font-medium">
                        ${option.price}/mo
