...
import { managementGoalOptions } from '@/data/socialMediaBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step1ManagementGoal() {
  const { managementGoal, setManagementGoal } = useSocialMediaBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What&apos;s your primary goal for social media?</h2>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {managementGoalOptions.map((option) => {
          const isSelected = managementGoal === option.id;

          return (
            <div key={option.id} className="relative">
              <motion.button
                onClick={() => setManagementGoal(option.id)}
                className={`
