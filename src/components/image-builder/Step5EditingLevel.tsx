...
import { editingOptions } from '@/data/imageBuilder';
import { Check, HelpCircle } from 'lucide-react';
import { useState } from 'react';

export default function Step5EditingLevel() {
  const { editing, setEditing } = useImageBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">How much editing and refinement is needed?</h2>
        <p className="text-[var(--text-secondary)]">
          Select the level of post-production work for your images.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        {editingOptions.map((option) => {
          const isSelected = editing === option.id;
