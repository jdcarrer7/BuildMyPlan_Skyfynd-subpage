...
import { quantityOptions } from '@/data/imageBuilder';
import { Check, HelpCircle, AlertCircle } from 'lucide-react';
import { useState } from 'react';

export default function Step2Quantity() {
  const { quantity, setQuantity } = useImageBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">How many images do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Select the quantity that fits your project scope.
        </p>
      </div>
