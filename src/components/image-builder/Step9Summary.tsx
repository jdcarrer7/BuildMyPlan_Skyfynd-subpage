...
import { useUnifiedQuoteStore, serviceMetadata, WebsiteConfig, AppConfig, AnimationConfig, SoundConfig, PaidMediaConfig, SocialMediaConfig, EmailMarketingConfig, VisualIdentityConfig, AIReceptionistConfig } from '@/hooks/useUnifiedQuoteStore';
import {
...
type ExpandedCard = 'image' | 'website' | 'app' | 'animation' | 'sound' | 'paid-media' | 'social-media' | 'email-marketing' | 'visual-identity' | 'ai-receptionist' | null;
...
export default function Step9Summary({ showQuoteForm = false, onCloseQuoteForm }: Step9SummaryProps) {
  ...
  useEffect(() => {
    // Only save if imageType is set (meaning we have a valid configuration)
    if (imageType) {
      saveToUnifiedQuote();
    }
  }, [imageType, saveToUnifiedQuote]);
  ...
  const handleClear = () => {
    ...
    // Clear from unified quote store first, then reset builder
    unifiedStore.clearServiceConfig('image');
    resetBuilder();

    // Delay navigation to ensure persist middleware has saved the state
    setTimeout(() => {
        ...
        {expandedCard === 'image' && (
          <div className="mt-6">
            <div className="space-y-4">
              {/* Image Type */}
              <SummarySection title="Image Type" icon={<Image className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={1}>
                <p className="text-[var(--text-secondary)]">{imageTypeLabel}</p>
              </SummarySection>

              {/* Quantity */}
              <SummarySection title="Quantity" icon={<Layers className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={2}>
              ...
              {/* Creation Method */}
              <SummarySection title="Creation Method" icon={<Sparkles className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={4}>
                <div className="flex justify-between">
              ...
              {/* Editing Level */}
              <SummarySection title="Editing Level" icon={<Palette className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={5}>
                <div className="flex justify-between">
              ...
              {/* Deliverables */}
              <SummarySection title="Deliverables" icon={<FileImage className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={7}>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
