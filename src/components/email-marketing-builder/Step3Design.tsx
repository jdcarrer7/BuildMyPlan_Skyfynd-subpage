...
import { designOptions } from '@/data/emailMarketingBuilder';
import { Check, HelpCircle, X, Palette } from 'lucide-react';
import { useState } from 'react';

export default function Step3Design() {
  const { design, setDesign } = useEmailMarketingBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const formatPrice = (option: typeof designOptions[0]) => {
    if (option.price === 0) {
      return 'Included';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What level of email design do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Choose the template and design approach that fits your brand and goals.
        </p>
      </div>
