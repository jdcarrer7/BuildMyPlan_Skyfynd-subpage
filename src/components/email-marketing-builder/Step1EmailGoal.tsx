...
export default function Step1EmailGoal() {
  const { emailGoal, setEmailGoal } = useEmailMarketingBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  // Define which options should have recommendation badge
  const recommendedOptions = ['newsletter', 'lead-nurturing', 'ecommerce'];

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What&apos;s your primary email marketing goal?</h2>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {emailGoalOptions.map((option) => {
          const isSelected = emailGoal === option.id;
          const isRecommended = recommendedOptions.includes(option.id);

          return (
            <div key={option.id} className="relative">
              <motion.button
                onClick={() => setEmailGoal(option.id)}
                className={`
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex flex-col gap-1">
                    ...
                    {isRecommended && (
                      <span className="text-xs text-[var(--accent-orange)]">Popular Choice</span>
                    )}
                  </div>
