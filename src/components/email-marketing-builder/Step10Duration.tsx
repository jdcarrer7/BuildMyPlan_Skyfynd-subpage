...
import { durationOptions } from '@/data/emailMarketingBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step10Duration() {
  const { duration, setDuration } = useEmailMarketingBuilderStore();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">How long do you want to commit?</h2>
        <p className="text-[var(--text-secondary)]">
          Longer commitments come with better pricing. Select your contract duration.
        </p>
      </div>
      ...
      >
        <h4 className="text-sm font-medium text-white mb-3">Pricing Impact by Duration</h4>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
          <div className="text-center">
            <span className="block text-red-400 font-medium">+15%</span>
            <span className="text-[var(--text-muted)]">Month-to-Month</span>
          </div>
          <div className="text-center">
            <span className="block text-gray-400 font-medium">0%</span>
