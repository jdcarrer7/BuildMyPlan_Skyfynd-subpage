...
import { copyOptions } from '@/data/emailMarketingBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step7Copy() {
  const { copy, setCopy } = useEmailMarketingBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">How should we handle email copywriting?</h2>
        <p className="text-[var(--text-secondary)]">
          Choose the level of copywriting and content support you need for your emails.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        {copyOptions.map((option) => {
          const isSelected = copy === option.id;

          return (
            <div key={option.id} className="relative">
              <motion.button
                onClick={() => setCopy(option.id)}
                className={`
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  {option.price === 0 ? (
                    ...
                    <>
                      <span className="text-lg font-semibold gradient-text">
                        {option.startsAt ? 'From ' : ''}${option.price}
                  ...
                  {!option.startsAt && option.price > 0 && (
