...
import { reportingOptions } from '@/data/emailMarketingBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step8Reporting() {
  const { reporting, setReporting } = useEmailMarketingBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">How do you want to track email performance?</h2>
        <p className="text-[var(--text-secondary)]">
          Select your preferred reporting frequency and level of analytics insights.
        </p>
      </div>
