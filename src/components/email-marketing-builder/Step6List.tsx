...
import { listOptions } from '@/data/emailMarketingBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step6List() {
  const { list, setList } = useEmailMarketingBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What level of list management do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Choose how we should organize, segment, and maintain your email list.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        {listOptions.map((option) => {
          const isSelected = list === option.id;
