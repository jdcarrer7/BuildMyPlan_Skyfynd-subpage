...
import { stationeryOptions, recommendationPresets } from '@/data/brandApplicationsBuilder';
import { Check, HelpCircle, X, FileText } from 'lucide-react';
import { useState } from 'react';

export default function Step2Stationery() {
  const { stationery, setStationery, applicationGoal } = useBrandApplicationsBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  // Get recommended stationery option based on application goal
  const recommendedStationery = applicationGoal ? recommendationPresets[applicationGoal]?.stationery : null;

  const formatPrice = (option: typeof stationeryOptions[0]) => {
    if (option.price === 0) {
      return 'Free';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">Stationery & Business Essentials</h2>
        <p className="text-[var(--text-secondary)]">
          Select the stationery package that fits your business needs.
        </p>
      </div>
      ...
      >
        {stationeryOptions.map((option) => {
          const isSelected = stationery === option.id;
          const isRecommended = recommendedStationery === option.id;

          return (
            <motion.div key={option.id} className="relative" variants={itemVariants}>
              <motion.button
                onClick={() => setStationery(option.id)}
                className={`
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <FileText className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
                    <div className="flex flex-col gap-1">
                      ...
                      {isRecommended && (
                        <span className="text-xs text-[var(--accent-orange)]">Recommended</span>
                      )}
                    </div>
                  </div>
                ...
                <div className="mt-2">
                  <div className="flex items-center gap-2">
                    {option.startsAt && (
                      <span className="text-xs px-2 py-0.5 bg-[var(--accent-orange)]/20 text-[var(--accent-orange)] rounded-full">
                        starting at
                      </span>
                    )}
                  </div>
