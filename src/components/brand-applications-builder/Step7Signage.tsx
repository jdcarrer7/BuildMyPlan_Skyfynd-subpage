...
import { signageOptions } from '@/data/brandApplicationsBuilder';
import { Check, HelpCircle, X, MapPin } from 'lucide-react';
import { useState } from 'react';

export default function Step7Signage() {
  const { signage, setSignage } = useBrandApplicationsBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const formatPrice = (option: typeof signageOptions[0]) => {
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What signage and environmental graphics do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Physical signage and environmental branding create a cohesive brand experience in your offices, retail spaces, and venues.
        </p>
      </div>
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <MapPin className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
