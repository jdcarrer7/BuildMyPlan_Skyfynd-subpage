...
import { presentationOptions } from '@/data/brandApplicationsBuilder';
import { Check, HelpCircle, X, Presentation } from 'lucide-react';
import { useState } from 'react';

export default function Step5Presentations() {
  const { presentations, setPresentations } = useBrandApplicationsBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const formatPrice = (option: typeof presentationOptions[0]) => {
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What presentation templates do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Professional presentation templates help you communicate your brand consistently in meetings, pitches, and proposals.
        </p>
      </div>
      ...
      >
        {presentationOptions.map((option) => {
          const isSelected = presentations === option.id;

          return (
            <motion.div key={option.id} className="relative" variants={itemVariants}>
              <motion.button
                onClick={() => setPresentations(option.id)}
                className={`
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <Presentation className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
