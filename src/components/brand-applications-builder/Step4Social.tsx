...
import { socialOptions, recommendationPresets } from '@/data/brandApplicationsBuilder';
import { Check, HelpCircle, X, Share2 } from 'lucide-react';
import { useState } from 'react';

export default function Step4Social() {
  const { social, setSocial, applicationGoal } = useBrandApplicationsBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  // Get recommended social option based on application goal
  const recommendedSocial = applicationGoal ? recommendationPresets[applicationGoal]?.social : null;

  const formatPrice = (option: typeof socialOptions[0]) => {
    if (option.price === 0) {
      return 'Free';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">Social Media Assets</h2>
        <p className="text-[var(--text-secondary)]">
          Select the social media assets package that fits your platform needs.
        </p>
      </div>
      ...
      >
        {socialOptions.map((option) => {
          const isSelected = social === option.id;
          const isRecommended = recommendedSocial === option.id;
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <Share2 className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
