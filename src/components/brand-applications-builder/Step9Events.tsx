...
import { eventsOptions } from '@/data/brandApplicationsBuilder';
import { Check, HelpCircle, X, Calendar } from 'lucide-react';
import { useState } from 'react';

export default function Step9Events() {
  const { events, setEvents } = useBrandApplicationsBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const formatPrice = (option: typeof eventsOptions[0]) => {
    if (option.price === 0) {
      return '$0';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">Do you need event and experiential materials?</h2>
        <p className="text-[var(--text-secondary)]">
          Create branded presence for trade shows, conferences, activations, and experiential marketing.
        </p>
      </div>

      <div className="flex items-center gap-2 text-sm text-[var(--text-muted)]">
        <Calendar className="w-4 h-4" />
        <span>Choose your events and experiential package</span>
      </div>
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <Calendar className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
