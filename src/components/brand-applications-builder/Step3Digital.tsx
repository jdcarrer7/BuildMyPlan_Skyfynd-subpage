...
import { digitalOptions, recommendationPresets } from '@/data/brandApplicationsBuilder';
import { Check, HelpCircle, X, Globe } from 'lucide-react';
import { useState } from 'react';

export default function Step3Digital() {
  const { digital, setDigital, applicationGoal } = useBrandApplicationsBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  // Get recommended digital option based on application goal
  const recommendedDigital = applicationGoal ? recommendationPresets[applicationGoal]?.digital : null;

  const formatPrice = (option: typeof digitalOptions[0]) => {
    if (option.price === 0) {
      return 'Free';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">Digital Assets & Templates</h2>
        <p className="text-[var(--text-secondary)]">
          Select the digital assets package that fits your online presence needs.
        </p>
      </div>
      ...
      >
        {digitalOptions.map((option) => {
          const isSelected = digital === option.id;
          const isRecommended = recommendedDigital === option.id;

          return (
            <motion.div key={option.id} className="relative" variants={itemVariants}>
              <motion.button
                onClick={() => setDigital(option.id)}
                className={`
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <Globe className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
