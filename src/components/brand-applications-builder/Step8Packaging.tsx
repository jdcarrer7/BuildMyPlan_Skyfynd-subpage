...
import { packagingOptions } from '@/data/brandApplicationsBuilder';
import { Check, HelpCircle, X, Package } from 'lucide-react';
import { useState } from 'react';

export default function Step8Packaging() {
  const { packaging, setPackaging } = useBrandApplicationsBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const formatPrice = (option: typeof packagingOptions[0]) => {
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What packaging and product design do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Branded packaging creates a memorable unboxing experience and reinforces your brand at every customer touchpoint.
        </p>
      </div>
      ...
      >
        {packagingOptions.map((option) => {
          const isSelected = packaging === option.id;
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <Package className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
