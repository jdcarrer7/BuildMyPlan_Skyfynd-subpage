...
import { platformOptions, platformTooltips } from '@/data/paidMediaBuilder';
import { Check, HelpCircle } from 'lucide-react';
import { useState } from 'react';

export default function Step2Platforms() {
  const { platforms, setPlatforms } = usePaidMediaBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">Which advertising platforms do you want to use?</h2>
        <p className="text-[var(--text-secondary)]">
          Select the platform or combination that best fits your target audience.
        </p>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {platformOptions.map((option) => {
          ...
          const tooltip = platformTooltips[option.id];

          return (
            <div key={option.id} className="relative">
              <motion.button
                onClick={() => setPlatforms(option.id)}
                className={`
              ...
              >
                ...
                <div className="text-lg font-semibold gradient-text">
                  {option.customQuote ? (
                    ...
                    <>
                      {option.startsAt && <span className="text-sm font-normal text-[var(--text-muted)]">Starting at </span>}
                      ${option.price}/mo
                    </>
                  )}
                </div>
