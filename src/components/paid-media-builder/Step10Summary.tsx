...
import { useUnifiedQuoteStore, serviceMetadata, WebsiteConfig, AppConfig, AnimationConfig, ImageConfig, SoundConfig, SocialMediaConfig, EmailMarketingConfig, VisualIdentityConfig, AIReceptionistConfig } from '@/hooks/useUnifiedQuoteStore';
import {
...
type ExpandedCard = 'paid-media' | 'website' | 'app' | 'animation' | 'image' | 'sound' | 'social-media' | 'email-marketing' | 'visual-identity' | 'ai-receptionist' | null;
...
export default function Step10Summary({ showQuoteForm = false, onCloseQuoteForm }: Step10SummaryProps) {
  const store = usePaidMediaBuilderStore();
  const unifiedStore = useUnifiedQuoteStore();
  const router = useRouter();

  const [expandedCard, setExpandedCard] = useState<ExpandedCard>('paid-media');
  const [showAddServices, setShowAddServices] = useState(false);

  // Save to unified quote on mount
  useEffect(() => {
    // Only save if campaignType is set (meaning we have a valid configuration)
    if (store.campaignType) {
      store.saveToUnifiedQuote();
    }
  }, [store.campaignType, store.saveToUnifiedQuote]);

  // Get labels
  const campaignTypeLabel = campaignTypeOptions.find((t) => t.id === store.campaignType)?.label;
  const platformLabel = platformOptions.find((p) => p.id === store.platforms)?.label;
  const budgetLabel = budgetOptions.find((b) => b.id === store.budget)?.label;
  const durationLabel = durationOptions.find((d) => d.id === store.duration)?.label;
  const managementLabel = managementOptions.find((m) => m.id === store.management)?.label;
  const creativesLabel = creativeOptions.find((c) => c.id === store.creatives)?.label;
  const landingPageLabel = landingPageOptions.find((l) => l.id === store.landingPage)?.label;
  const trackingLabel = trackingOptions.find((t) => t.id === store.tracking)?.label;
  const reportingLabel = reportingOptions.find((r) => r.id === store.reporting)?.label;

  // Get available and configured services
  const availableServices = unifiedStore.getAvailableServices().filter((s) => s !== 'paid-media');
  const configuredServices = unifiedStore.getAllConfiguredServices();
  const otherServices = configuredServices.filter((s) => s.type !== 'paid-media');
  ...
  const handleClear = () => {
    ...
    const remainingServices = configuredServices.filter(s => s.type !== 'paid-media');

    // Clear from unified quote store first, then builder store
    unifiedStore.clearServiceConfig('paid-media');
    store.resetBuilder();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[var(--text-secondary)] text-sm">{campaignTypeLabel}</p>
            </div>
          </div>
        ...
        <div className={`grid transition-all duration-300 ease-in-out ${expandedCard === 'paid-media' ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
          <div className="overflow-hidden">
            <div className="space-y-4 mt-6">
                <SummarySection title="Campaign Type" icon={<Target className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={1}>
                ...
                <SummarySection title="Ad Spend Budget" icon={<DollarSign className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={3}>
                  <div className="flex justify-between">
                    <span className="text-[var(--text-secondary)]">{budgetLabel}</span>
                    <span className="text-[var(--text-muted)] text-sm">Paid to platforms</span>
                  </div>
                </SummarySection>

                <SummarySection title="Duration" icon={<Calendar className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={4}>
                  <div className="flex justify-between">
                ...
                <SummarySection title="Management" icon={<Settings className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={5}>
                  <div className="flex justify-between">
                    <span className="text-[var(--text-secondary)]">{managementLabel}</span>
                    <span className={store.managementOneTime ? 'text-white' : 'text-[var(--accent-pink)]'}>
                      ${store.managementPrice}{store.managementOneTime ? '' : '/mo'}
                    </span>
                  </div>
                </SummarySection>

                <SummarySection title="Creatives" icon={<Palette className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={6}>
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-[var(--text-secondary)]">{creativesLabel}</span>
                      {store.creativesPrice > 0 ? (
                        <span className="text-white">${store.creativesPrice}</span>
                      ) : (
                        <span className="text-[var(--text-muted)]">$0</span>
                      )}
                    </div>
                    {store.landingPage && store.landingPage !== 'none' && (
                      <div className="flex justify-between text-sm">
                        <span className="text-[var(--text-secondary)]">{landingPageLabel}</span>
                        <span className="text-white">${store.landingPagePrice}</span>
                      </div>
                    )}
                  </div>
                </SummarySection>

                <SummarySection title="Tracking" icon={<BarChart3 className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={7}>
                  <div className="flex justify-between">
                    <span className="text-[var(--text-secondary)]">{trackingLabel}</span>
                    <span className="text-white">${store.trackingPrice}</span>
                  </div>
                </SummarySection>

                <SummarySection title="Reporting" icon={<FileText className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={8}>
                  <div className="flex justify-between">
                ...
                <SummarySection title="Add-ons" icon={<Zap className="w-5 h-5 text-[var(--accent-purple)]" />} stepNumber={9}>
                  {store.selectedAddOns.length === 0 ? (
                    <p className="text-[var(--text-muted)] text-sm">No add-ons selected</p>
                  ) : (
                    <div className="space-y-2">
                      {store.selectedAddOns.map((addOn) => {
                        const addOnData = addOnOptions.find((a) => a.id === addOn.id);
                        return (
