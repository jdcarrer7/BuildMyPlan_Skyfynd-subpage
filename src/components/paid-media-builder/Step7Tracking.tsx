...
import { trackingOptions } from '@/data/paidMediaBuilder';
import { Check, HelpCircle } from 'lucide-react';
import { useState } from 'react';

export default function Step7Tracking() {
  const { tracking, setTracking } = usePaidMediaBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What level of tracking do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Choose the tracking and analytics setup for your campaigns.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        {trackingOptions.map((option) => {
          const isSelected = tracking === option.id;
