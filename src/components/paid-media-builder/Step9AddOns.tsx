...
import { addOnOptions } from '@/data/paidMediaBuilder';
import { Check, HelpCircle, Plus } from 'lucide-react';
import { useState } from 'react';

export default function Step9AddOns() {
  const { selectedAddOns, toggleAddOn } = usePaidMediaBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">Enhance your campaign with add-ons</h2>
        <p className="text-[var(--text-secondary)]">
          Select any additional services to maximize your campaign performance.
          <span className="text-[var(--text-muted)] ml-1">(Select multiple)</span>
        </p>
      </div>
      ...
      {selectedAddOns.length > 0 && (
        ...
        >
          <p className="text-sm text-[var(--text-secondary)]">
            <span className="text-white font-medium">{selectedAddOns.length} add-on{selectedAddOns.length > 1 ? 's' : ''}</span> selected
          </p>
        </motion.div>
      )}
    </div>
