...
import { soundOptions } from '@/data/animationBuilder';
import { HelpCircle, Check, Volume2 } from 'lucide-react';
import { useState } from 'react';

export default function Step5Sound() {
  const { sound, setSound } = useAnimationBuilderStore();
    ...
    <div className="card p-6">
      <div className="mb-6">
        ...
        <p className="text-[var(--text-secondary)]">
          Great sound design elevates your animation. Choose what fits your project.
        </p>
      </div>

      <div className="grid gap-3">
        {soundOptions.map((option) => {
          const isSelected = sound === option.id;
          ...
          return (
            <div key={option.id} className="relative">
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <span className={`font-semibold ${option.price === 0 ? 'text-[var(--text-muted)]' : 'text-white'}`}>
                    {option.price === 0 ? '$0' : option.startsAt ? `$${option.price}+` : `$${option.price}`}
                  </span>
                  <span
                    onClick={(e) => {
                      e.stopPropagation();
