...
import { styleOptions } from '@/data/animationBuilder';
import { HelpCircle, Check, Palette } from 'lucide-react';
import { useState } from 'react';

export default function Step3Style() {
  const { style, setStyle } = useAnimationBuilderStore();
          ...
          return (
            <div key={option.id} className="relative">
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <span className={`font-semibold ${option.customQuote ? 'text-[var(--accent-orange)]' : 'text-white'}`}>
                    {option.price === null ? 'Custom Quote' : option.startsAt ? `$${option.price}+` : `$${option.price}`}
                  </span>
                  <span
                    onClick={(e) => {
                      e.stopPropagation();
