...
import { timelineOptions } from '@/data/animationBuilder';
import { HelpCircle, Check, Clock, AlertTriangle } from 'lucide-react';
import { useState } from 'react';

export default function Step8Timeline() {
  const { timeline, setTimeline } = useAnimationBuilderStore();
    ...
    <div className="card p-6">
      <div className="mb-6">
        ...
        <p className="text-[var(--text-secondary)]">
          Select your preferred timeline. Faster delivery may affect pricing.
        </p>
      </div>
      ...
      <div className="grid gap-3">
        {timelineOptions.map((option) => {
          ...
          const hasMultiplier = option.multiplier > 1;
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {hasMultiplier && (
                      <span className="flex items-center gap-1 text-xs bg-[var(--accent-orange)]/20 text-[var(--accent-orange)] px-2 py-0.5 rounded-full">
      ...
      {/* Timeline note */}
      <div className="mt-6 p-4 bg-[var(--bg-secondary)] border border-[var(--border-subtle)] rounded-lg">
        <p className="text-sm text-[var(--text-muted)]">
          <strong className="text-[var(--text-secondary)]">Note:</strong> Rush and expedited timelines are subject to availability.
          We&apos;ll confirm feasibility after reviewing your project details.
        </p>
      </div>
    </div>
  );
