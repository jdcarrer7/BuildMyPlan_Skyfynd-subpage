...
import { strategyGoalOptions } from '@/data/brandStrategyBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step1StrategyGoal() {
  const { strategyGoal, setStrategyGoal } = useBrandStrategyBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What&apos;s driving your need for brand strategy?</h2>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {strategyGoalOptions.map((option) => {
          const isSelected = strategyGoal === option.id;

          return (
            <div key={option.id} className="relative">
              <motion.button
                onClick={() => setStrategyGoal(option.id)}
                className={`
