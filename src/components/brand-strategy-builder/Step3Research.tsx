...
import { researchOptions } from '@/data/brandStrategyBuilder';
import { Check, HelpCircle, X, Search } from 'lucide-react';
import { useState } from 'react';

export default function Step3Research() {
  const { research, setResearch } = useBrandStrategyBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const formatPrice = (option: typeof researchOptions[0]) => {
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What level of research should inform your strategy?</h2>
        <p className="text-[var(--text-secondary)]">
          Choose the research depth that will provide the insights needed for your brand decisions.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {researchOptions.map((option) => {
          const isSelected = research === option.id;

          return (
            <div key={option.id} className="relative">
              <motion.button
                onClick={() => setResearch(option.id)}
                className={`
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <Search className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
