...
import { positioningOptions } from '@/data/brandStrategyBuilder';
import { Check, HelpCircle, X, Target } from 'lucide-react';
import { useState } from 'react';

export default function Step4Positioning() {
  const { positioning, setPositioning } = useBrandStrategyBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const formatPrice = (option: typeof positioningOptions[0]) => {
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What level of positioning work do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Define how your brand differentiates and communicates its unique value in the market.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {positioningOptions.map((option) => {
          const isSelected = positioning === option.id;
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <Target className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
