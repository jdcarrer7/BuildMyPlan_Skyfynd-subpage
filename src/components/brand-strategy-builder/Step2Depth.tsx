...
import { depthOptions, recommendationPresets } from '@/data/brandStrategyBuilder';
import { Check, HelpCircle, X, Layers } from 'lucide-react';
import { useState } from 'react';

export default function Step2Depth() {
  const { depth, setDepth, strategyGoal } = useBrandStrategyBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  // Get recommended depth based on strategy goal
  const recommendedDepth = strategyGoal ? recommendationPresets[strategyGoal]?.depth : null;
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">How comprehensive does your brand strategy need to be?</h2>
