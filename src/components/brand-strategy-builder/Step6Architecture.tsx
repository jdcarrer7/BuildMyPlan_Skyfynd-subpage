...
import { architectureOptions } from '@/data/brandStrategyBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step6Architecture() {
  const { architecture, setArchitecture } = useBrandStrategyBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  // Format price display
  const formatPrice = (option: typeof architectureOptions[0]) => {
    if (option.price === 0) {
      return '$0';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">Do you need brand architecture guidance?</h2>
        <p className="text-[var(--text-secondary)]">
          Brand architecture defines how your brands, sub-brands, and products relate to each other.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        {architectureOptions.map((option) => {
          const isSelected = architecture === option.id;
