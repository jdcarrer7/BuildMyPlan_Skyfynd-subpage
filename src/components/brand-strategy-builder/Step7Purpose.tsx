...
import { purposeOptions } from '@/data/brandStrategyBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step7Purpose() {
  const { purpose, setPurpose } = useBrandStrategyBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  // Format price display
  const formatPrice = (option: typeof purposeOptions[0]) => {
    if (option.price === 0) {
      return '$0';
    }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">Do you need purpose, vision, and values development?</h2>
        <p className="text-[var(--text-secondary)]">
          Purpose, vision, and values form the foundation of your brand culture and long-term direction.
        </p>
      </div>
