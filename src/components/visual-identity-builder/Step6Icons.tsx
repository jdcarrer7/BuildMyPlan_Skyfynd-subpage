...
import { iconOptions } from '@/data/visualIdentityBuilder';
import { Check, HelpCircle, X, Shapes } from 'lucide-react';
import { useState } from 'react';

export default function Step6Icons() {
  const { icons, setIcons } = useVisualIdentityBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const formatPrice = (option: typeof iconOptions[0]) => {
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What icon system do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Custom icons help reinforce your brand identity across digital touchpoints and marketing materials.
        </p>
      </div>
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <Shapes className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
