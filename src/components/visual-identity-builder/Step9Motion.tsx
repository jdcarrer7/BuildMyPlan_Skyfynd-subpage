...
import { motionOptions } from '@/data/visualIdentityBuilder';
import { Check, HelpCircle, X, Play } from 'lucide-react';
import { useState } from 'react';

export default function Step9Motion() {
  const { motion: selectedMotion, setMotion } = useVisualIdentityBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const formatPrice = (option: typeof motionOptions[0]) => {
    if (option.price === 0) {
      return '$0';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">Do you need motion and animation guidelines?</h2>
        <p className="text-[var(--text-secondary)]">
          Motion design brings your brand to life in digital experiences with consistent, purposeful animation.
        </p>
      </div>
      ...
      >
        {motionOptions.map((option) => {
          const isSelected = selectedMotion === option.id;
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <Play className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
