...
import { guidelinesOptions } from '@/data/visualIdentityBuilder';
import { Check, HelpCircle, X, BookOpen } from 'lucide-react';
import { useState } from 'react';

export default function Step10Guidelines() {
  const { guidelines, setGuidelines } = useVisualIdentityBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const formatPrice = (option: typeof guidelinesOptions[0]) => {
    if (option.price === 0) {
      return '$0';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What level of brand guidelines documentation do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Brand guidelines ensure consistent application of your visual identity across all touchpoints and teams.
        </p>
      </div>
      ...
      <motion.div
        ...
        animate="visible"
      >
        {guidelinesOptions.map((option) => {
          const isSelected = guidelines === option.id;

          return (
            <motion.div key={option.id} className="relative" variants={itemVariants}>
              <motion.button
                onClick={() => setGuidelines(option.id)}
                className={`
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <BookOpen className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
