...
interface VisualIdentityStepIndicatorProps {
  currentStep: number;
  totalSteps: number;
...
export default function VisualIdentityStepIndicator({
  currentStep,
  totalSteps,
  onStepClick,
}: VisualIdentityStepIndicatorProps) {
  ...
  return (
    <div className="relative">
      ...
      <div className="absolute -top-8 left-1/2 -translate-x-1/2 flex items-center gap-2 px-3 py-1 bg-[var(--bg-secondary)] rounded-full border border-[var(--border-subtle)]">
        <Palette className="w-3 h-3 text-[var(--accent-blue)]" />
        <span className="text-xs text-[var(--text-secondary)]">Visual Identity Builder</span>
      </div>
    </div>
  );
}
