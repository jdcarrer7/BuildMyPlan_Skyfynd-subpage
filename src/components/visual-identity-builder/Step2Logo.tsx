...
import { logoOptions, recommendationPresets } from '@/data/visualIdentityBuilder';
import { Check, HelpCircle, X, Sparkles } from 'lucide-react';
import { useState } from 'react';

export default function Step2Logo() {
  const { logo, setLogo, visualIdentityGoal } = useVisualIdentityBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  // Get recommended logo option based on visual identity goal
  const recommendedLogo = visualIdentityGoal ? recommendationPresets[visualIdentityGoal]?.logo : null;

  const formatPrice = (option: typeof logoOptions[0]) => {
    if (option.price === 0) {
      return 'Free';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What logo work do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Select the logo design package that fits your requirements.
        </p>
      </div>
      ...
      >
        {logoOptions.map((option) => {
          const isSelected = logo === option.id;
          const isRecommended = recommendedLogo === option.id;
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <Sparkles className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
