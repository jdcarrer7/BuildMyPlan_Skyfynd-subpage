...
import { colorOptions, recommendationPresets } from '@/data/visualIdentityBuilder';
import { Check, HelpCircle, X, Palette } from 'lucide-react';
import { useState } from 'react';

export default function Step3Colors() {
  const { colors, setColors, visualIdentityGoal } = useVisualIdentityBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  // Get recommended color option based on visual identity goal
  const recommendedColors = visualIdentityGoal ? recommendationPresets[visualIdentityGoal]?.colors : null;

  const formatPrice = (option: typeof colorOptions[0]) => {
    if (option.price === 0) {
      return 'Free';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What color system do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Select the color palette development that matches your brand needs.
        </p>
      </div>
      ...
      >
        {colorOptions.map((option) => {
          const isSelected = colors === option.id;
          const isRecommended = recommendedColors === option.id;

          return (
            <motion.div key={option.id} className="relative" variants={itemVariants}>
              <motion.button
                onClick={() => setColors(option.id)}
                className={`
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <Palette className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
