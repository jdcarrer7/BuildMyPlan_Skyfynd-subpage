...
import { illustrationOptions } from '@/data/visualIdentityBuilder';
import { Check, HelpCircle, X, Pencil } from 'lucide-react';
import { useState } from 'react';

export default function Step8Illustration() {
  const { illustration, setIllustration } = useVisualIdentityBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const formatPrice = (option: typeof illustrationOptions[0]) => {
    if (option.price === 0) {
      return '$0';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">Do you need illustration style development?</h2>
        <p className="text-[var(--text-secondary)]">
          Custom illustrations can bring personality and storytelling to your brand across touchpoints.
        </p>
      </div>
      ...
      >
        {illustrationOptions.map((option) => {
          const isSelected = illustration === option.id;

          return (
            <motion.div key={option.id} className="relative" variants={itemVariants}>
              <motion.button
                onClick={() => setIllustration(option.id)}
                className={`
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <Pencil className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
