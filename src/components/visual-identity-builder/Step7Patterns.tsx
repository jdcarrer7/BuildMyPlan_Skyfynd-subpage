...
import { patternOptions } from '@/data/visualIdentityBuilder';
import { Check, HelpCircle, X, Grid3X3 } from 'lucide-react';
import { useState } from 'react';

export default function Step7Patterns() {
  const { patterns, setPatterns } = useVisualIdentityBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const formatPrice = (option: typeof patternOptions[0]) => {
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What pattern & texture system do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Patterns and textures add depth and visual interest to your brand applications.
        </p>
      </div>
