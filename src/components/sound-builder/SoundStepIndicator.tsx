...
interface SoundStepIndicatorProps {
  currentStep: number;
  totalSteps: number;
...
export default function SoundStepIndicator({ currentStep, totalSteps, onStepClick }: SoundStepIndicatorProps) {
  return (
    <div className="w-full mb-8">
      ...
      <div className="hidden md:flex items-center mb-4">
        <div className="flex items-center gap-2 px-3 py-1.5 bg-[var(--bg-card)] border border-[var(--border-subtle)] rounded-full">
          <Music className="w-4 h-4 text-[var(--accent-blue)]" />
          <span className="text-sm font-medium text-white">Brand Signature Sound</span>
        </div>
      </div>
