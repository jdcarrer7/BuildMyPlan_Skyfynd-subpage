...
import { durationOptions } from '@/data/soundBuilder';
import { Check, HelpCircle } from 'lucide-react';
import { useState } from 'react';

export default function Step2Duration() {
  const { duration, setDuration } = useSoundBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">How long should your sound be?</h2>
        <p className="text-[var(--text-secondary)]">
          Select the duration that best fits your project needs.
        </p>
      </div>
