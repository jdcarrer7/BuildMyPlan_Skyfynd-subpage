...
import { timelineOptions } from '@/data/soundBuilder';
...
export default function Step8Timeline() {
  const { timeline, setTimeline, subtotal } = useSoundBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">When do you need it?</h2>
        <p className="text-[var(--text-secondary)]">
          Select your preferred timeline. Rush options include priority production fees.
        </p>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {timelineOptions.map((option) => {
          ...
          const rushFee = option.multiplier > 1 ? Math.round(subtotal * (option.multiplier - 1)) : 0;
              ...
              >
                ...
                <div className="text-lg font-semibold">
                  {rushFee > 0 ? (
                    ...
                    <span className="text-[var(--accent-blue-light)]">Standard pricing</span>
                  )}
                </div>
              </motion.button>
