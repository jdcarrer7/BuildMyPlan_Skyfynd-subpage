...
import { quantityOptions } from '@/data/soundBuilder';
import { Check, HelpCircle } from 'lucide-react';
import { useState } from 'react';

export default function Step5Quantity() {
  const { quantity, setQuantity } = useSoundBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">How many sounds or variations do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Select the number of sound variations or individual pieces for your project.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        {quantityOptions.map((option) => {
          const isSelected = quantity === option.id;
            ...
            <div key={option.id} className="relative">
              ...
              >
                ...
                <div className="text-lg font-semibold gradient-text">
                  {option.price === null ? 'Custom Quote' : option.price === 0 ? 'Included' : `+$${option.price}`}
                </div>
              </motion.button>
