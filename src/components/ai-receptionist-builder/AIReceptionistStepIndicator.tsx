...
interface AIReceptionistStepIndicatorProps {
  currentStep: number;
  totalSteps: number;
...
export default function AIReceptionistStepIndicator({
  currentStep,
  totalSteps,
  onStepClick,
}: AIReceptionistStepIndicatorProps) {
  ...
  return (
    <div className="relative">
      ...
      <div className="absolute -top-8 left-1/2 -translate-x-1/2 flex items-center gap-2 px-3 py-1 bg-[var(--bg-secondary)] rounded-full border border-[var(--border-subtle)]">
        <Phone className="w-3 h-3 text-[var(--accent-purple)]" />
        <span className="text-xs text-[var(--text-secondary)]">AI Receptionist Builder</span>
      </div>
    </div>
  );
}
