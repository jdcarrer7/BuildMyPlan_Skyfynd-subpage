...
import { marketingOptions, marketingTooltips } from '@/data/messagingCopywritingBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step5MarketingCopy() {
  const { marketing, setMarketing } = useMessagingCopywritingBuilderStore();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What marketing copy do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Select the marketing and advertising copy package for your campaigns.
        </p>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {marketingOptions.map((option) => {
          ...
          const tooltip = marketingTooltips[option.id];
