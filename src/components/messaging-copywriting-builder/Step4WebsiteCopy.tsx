...
import { websiteOptions, websiteTooltips } from '@/data/messagingCopywritingBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step4WebsiteCopy() {
  const { website, setWebsite } = useMessagingCopywritingBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const formatPrice = (option: typeof websiteOptions[0]) => {
    if (option.price === 0) {
      return 'Not Needed';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What website copy do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Select the website copy package that fits your site structure.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        {websiteOptions.map((option) => {
          const isSelected = website === option.id;
          const tooltip = websiteTooltips[option.id];

          return (
            <div key={option.id} className="relative">
              <motion.button
                onClick={() => setWebsite(option.id)}
                className={`
