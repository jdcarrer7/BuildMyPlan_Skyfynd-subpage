...
import { addOnServices, addOnTooltips } from '@/data/messagingCopywritingBuilder';
import { Check, HelpCircle, Plus, Sparkles } from 'lucide-react';
import { useState } from 'react';

export default function Step11AddOns() {
  const { selectedAddOns, toggleAddOn } = useMessagingCopywritingBuilderStore();
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[var(--text-secondary)]">
          Enhance your copywriting package with specialized add-on services. All add-ons are one-time fees.
        </p>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {addOnServices.map((option) => {
          ...
          const tooltip = addOnTooltips[option.id];
