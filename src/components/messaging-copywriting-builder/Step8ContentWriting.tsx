...
import { contentOptions, contentTooltips } from '@/data/messagingCopywritingBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step8ContentWriting() {
  const { content, setContent } = useMessagingCopywritingBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  // Format price display
  const formatPrice = (option: typeof contentOptions[0]) => {
    if (option.price === 0) {
      return 'Included';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What content writing do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Quality content builds authority, drives organic traffic, and engages your audience.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        {contentOptions.map((option) => {
          const isSelected = content === option.id;
          const tooltip = contentTooltips[option.id];

          return (
            <div key={option.id} className="relative">
              <motion.button
                onClick={() => setContent(option.id)}
                className={`
