...
import { uxOptions, uxTooltips } from '@/data/messagingCopywritingBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step9UxWriting() {
  const { ux, setUx } = useMessagingCopywritingBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  // Format price display
  const formatPrice = (option: typeof uxOptions[0]) => {
    if (option.price === 0) {
      return 'Included';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What UX writing do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          UX writing creates clear, helpful microcopy that guides users through your digital products.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        {uxOptions.map((option) => {
          const isSelected = ux === option.id;
          const tooltip = uxTooltips[option.id];

          return (
            <div key={option.id} className="relative">
              <motion.button
                onClick={() => setUx(option.id)}
                className={`
