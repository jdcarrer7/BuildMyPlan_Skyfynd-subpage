...
import { productOptions, productTooltips } from '@/data/messagingCopywritingBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step7ProductCopy() {
  const { product, setProduct } = useMessagingCopywritingBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  // Format price display
  const formatPrice = (option: typeof productOptions[0]) => {
    if (option.price === 0) {
      return 'Included';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What product copy do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Product copy brings your offerings to life with compelling descriptions that convert.
        </p>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {productOptions.map((option) => {
          ...
          const tooltip = productTooltips[option.id];
