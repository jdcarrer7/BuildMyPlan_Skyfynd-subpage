...
import { messagingOptions, messagingTooltips } from '@/data/messagingCopywritingBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step2MessagingFramework() {
  const { messaging, setMessaging } = useMessagingCopywritingBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const formatPrice = (option: typeof messagingOptions[0]) => {
    if (option.price === 0) {
      return 'Not Needed';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What level of messaging foundation do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Select the messaging framework depth that matches your brand needs.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        {messagingOptions.map((option) => {
          const isSelected = messaging === option.id;
          const tooltip = messagingTooltips[option.id];

          return (
            <div key={option.id} className="relative">
              <motion.button
                onClick={() => setMessaging(option.id)}
                className={`
