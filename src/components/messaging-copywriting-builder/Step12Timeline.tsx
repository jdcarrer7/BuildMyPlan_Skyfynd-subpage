...
import { timelineOptions, timelineTooltips } from '@/data/messagingCopywritingBuilder';
import { Check, HelpCircle, Clock, Zap, AlertTriangle, Info } from 'lucide-react';
...
export default function Step12Timeline() {
  const { timeline, setTimeline, oneTimeSubtotal } = useMessagingCopywritingBuilderStore();
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-2 p-3 bg-[var(--bg-secondary)] rounded-lg">
        ...
        <span className="text-sm text-[var(--text-secondary)]">
          Timeline applies to one-time project work. Retainer is ongoing monthly.
        </span>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {timelineOptions.map((option) => {
          ...
          const tooltip = timelineTooltips[option.id];
          const premiumAmount = option.multiplier > 1 ? Math.round(oneTimeSubtotal * (option.multiplier - 1)) : 0;
            ...
            <div key={option.id} className="relative">
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  ...
                  {premiumAmount > 0 && oneTimeSubtotal > 0 && (
