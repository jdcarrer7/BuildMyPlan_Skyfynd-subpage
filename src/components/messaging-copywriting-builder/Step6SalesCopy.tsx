...
import { salesOptions, salesTooltips } from '@/data/messagingCopywritingBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step6SalesCopy() {
  const { sales, setSales } = useMessagingCopywritingBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  // Format price display
  const formatPrice = (option: typeof salesOptions[0]) => {
    if (option.price === 0) {
      return 'Included';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What sales materials do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Sales enablement copy helps your team close more deals with compelling materials.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        {salesOptions.map((option) => {
          const isSelected = sales === option.id;
          const tooltip = salesTooltips[option.id];

          return (
            <div key={option.id} className="relative">
              <motion.button
                onClick={() => setSales(option.id)}
                className={`
