...
import { messagingGoalOptions } from '@/data/messagingCopywritingBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step1MessagingGoal() {
  const { messagingGoal, setMessagingGoal, applyRecommendations } = useMessagingCopywritingBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const handleGoalSelect = (goalId: string) => {
    setMessagingGoal(goalId);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What&apos;s driving your need for messaging and copywriting?</h2>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {messagingGoalOptions.map((option) => {
          const isSelected = messagingGoal === option.id;
