...
import { projectTypes } from '@/data/websiteBuilder';
import { Check, Info, X } from 'lucide-react';

export default function Step1ProjectType() {
  const { projectType, setProjectType, nextStep } = useBuilderStore();
  ...
  const selectedTooltip = tooltipType
    ? projectTypes.find(t => t.id === tooltipType)?.tooltip
    : null;
          ...
          >
            <div className="flex justify-between items-start mb-3">
              <h4 className="font-semibold text-white">
                {projectTypes.find(t => t.id === tooltipType)?.label}
              </h4>
              <button
