...
import { measurementOptions, recommendationPresets } from '@/data/contentStrategyBuilder';
import { Check, HelpCircle, X, BarChart3 } from 'lucide-react';
import { useState } from 'react';

export default function Step9Measurement() {
  const { measurement, setMeasurement, contentGoal } = useContentStrategyBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  // Get recommended measurement based on content goal
  const recommendedMeasurement = contentGoal ? recommendationPresets[contentGoal]?.measurement : null;

  const formatPrice = (option: typeof measurementOptions[0]) => {
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What level of content measurement framework do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Select the analytics and measurement approach that aligns with your performance goals.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {measurementOptions.map((option) => {
          const isSelected = measurement === option.id;
          const isRecommended = recommendedMeasurement === option.id;

          return (
            <div key={option.id} className="relative">
              <motion.button
                onClick={() => setMeasurement(option.id)}
                className={`
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <BarChart3 className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
