...
import { contentGoalOptions } from '@/data/contentStrategyBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step1ContentGoal() {
  const { contentGoal, setContentGoal, applyRecommendations } = useContentStrategyBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const handleGoalSelect = (goalId: string) => {
    setContentGoal(goalId);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What&apos;s driving your need for content strategy?</h2>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {contentGoalOptions.map((option) => {
          const isSelected = contentGoal === option.id;
