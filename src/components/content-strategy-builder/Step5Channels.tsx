...
import { channelOptions } from '@/data/contentStrategyBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step5Channels() {
  const { channels, setChannels } = useContentStrategyBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  // Format price display
  const formatPrice = (option: typeof channelOptions[0]) => {
    if (option.price === 0) {
      return '$0';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">How many channels does your content strategy need to cover?</h2>
        <p className="text-[var(--text-secondary)]">
          Select the number of content channels your strategy should address.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        {channelOptions.map((option) => {
          const isSelected = channels === option.id;
