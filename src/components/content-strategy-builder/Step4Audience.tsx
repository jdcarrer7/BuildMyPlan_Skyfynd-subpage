...
import { audienceOptions } from '@/data/contentStrategyBuilder';
import { Check, HelpCircle, X, Users } from 'lucide-react';
import { useState } from 'react';

export default function Step4Audience() {
  const { audience, setAudience } = useContentStrategyBuilderStore();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What level of audience work do you need for content?</h2>
        <p className="text-[var(--text-secondary)]">
          Understanding your audience helps create more effective, targeted content.
        </p>
      </div>
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <Users className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
