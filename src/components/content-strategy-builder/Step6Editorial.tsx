...
import { editorialOptions } from '@/data/contentStrategyBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step6Editorial() {
  const { editorial, setEditorial } = useContentStrategyBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  // Format price display
  const formatPrice = (option: typeof editorialOptions[0]) => {
    if (option.price === 0) {
      return '$0';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What level of editorial planning do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          Editorial planning helps structure your content creation and publication workflow.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        {editorialOptions.map((option) => {
          const isSelected = editorial === option.id;

          return (
            <div key={option.id} className="relative">
              <motion.button
                onClick={() => setEditorial(option.id)}
                className={`
