...
import { depthOptions, recommendationPresets } from '@/data/contentStrategyBuilder';
import { Check, HelpCircle, X, Layers } from 'lucide-react';
import { useState } from 'react';

export default function Step2Depth() {
  const { depth, setDepth, contentGoal } = useContentStrategyBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  // Get recommended depth based on content goal
  const recommendedDepth = contentGoal ? recommendationPresets[contentGoal]?.depth : null;

  const formatPrice = (option: typeof depthOptions[0]) => {
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">How comprehensive does your content strategy need to be?</h2>
        <p className="text-[var(--text-secondary)]">
          Select the depth of strategy work that matches your needs and budget.
        </p>
      </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {depthOptions.map((option) => {
          ...
          const isRecommended = recommendedDepth === option.id;
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <Layers className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
