...
import { seoOptions } from '@/data/contentStrategyBuilder';
import { Check, HelpCircle, X } from 'lucide-react';
import { useState } from 'react';

export default function Step7Seo() {
  const { seo, setSeo } = useContentStrategyBuilderStore();
  const [tooltipOpen, setTooltipOpen] = useState<string | null>(null);

  // Format price display
  const formatPrice = (option: typeof seoOptions[0]) => {
    if (option.price === 0) {
      return '$0';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">What level of SEO strategy do you need?</h2>
        <p className="text-[var(--text-secondary)]">
          SEO strategy helps your content get discovered through organic search.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        {seoOptions.map((option) => {
          const isSelected = seo === option.id;

          return (
            <div key={option.id} className="relative">
              <motion.button
                onClick={() => setSeo(option.id)}
                className={`
