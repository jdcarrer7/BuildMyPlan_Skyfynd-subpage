...
import { auditOptions } from '@/data/contentStrategyBuilder';
import { Check, HelpCircle, X, ClipboardCheck } from 'lucide-react';
import { useState } from 'react';

export default function Step3Audit() {
  const { audit, setAudit } = useContentStrategyBuilderStore();
  const [activeTooltip, setActiveTooltip] = useState<string | null>(null);

  const formatPrice = (option: typeof auditOptions[0]) => {
    if (option.price === 0) {
      return '$0';
    }
    if (option.startsAt) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white mb-2">Do you need existing content audited?</h2>
        <p className="text-[var(--text-secondary)]">
          A content audit helps identify what you have, what's working, and what needs improvement.
        </p>
      </div>

      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {auditOptions.map((option) => {
          const isSelected = audit === option.id;

          return (
            <div key={option.id} className="relative">
              <motion.button
                onClick={() => setAudit(option.id)}
                className={`
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    `}>
                      <ClipboardCheck className={`w-5 h-5 ${isSelected ? 'text-[var(--accent-blue)]' : 'text-[var(--text-secondary)]'}`} />
                    </div>
